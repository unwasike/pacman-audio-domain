...
void record_audio(u32* left_audio_memory, u32* right_audio_memory)
{
	u32  in_left, in_right;
	...
	while (samples_left > 0) {
		...
		Xil_Out32(I2S_DATA_TX_L_REG, in_left);
		Xil_Out32(I2S_DATA_TX_R_REG, in_right);
		*(left_audio_memory_ptr) = in_left;
		*(right_audio_memory_ptr) = in_right;
...
void playback_audio_spedup(u32* left_audio_memory, u32* right_audio_memory)
{
	...
	while(samples_left > 0) {
		...
		// Shorter delay for fast playback
		usleep(10);

	}
}

void playback_audio_sloweddown(u32* left_audio_memory, u32* right_audio_memory)
{
	...
	while(samples_left > 0) {
		...
		// Longer delay for slow playback
		usleep(40);
